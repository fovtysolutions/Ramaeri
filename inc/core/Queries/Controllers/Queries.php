<?php
namespace Core\Queries\Controllers;
use App\Controllers\PeginationController;

class Queries extends \CodeIgniter\Controller
{
    public function __construct(){
        $this->config = include realpath( __DIR__."/../Config.php" );
        $this->model = new \Core\Queries\Models\QueriesModel();
    ...
    public function index()
    {
        ...
        $mainDetails = [   
            'dashboard' => $master,
            'title' => $master, 
            'heading' => "Queries List", 
            'print' => '', 
            'searchname' => "Search $master", 
            'searching'=> '',
            'filterinput'=> '',
            'filterids'=>json_encode([]),
            'th'=>["Name", "Email", "Phone","Message", "Issue"],
            'mainid'=> "get$master",
            'addbtnroute'=> "$master/add",
            'routeURL'=> "$master/get",
            'editroute'=> "/$master/edit/",
            'deleteURL'=> "$master/deleteit",
            'td'=>json_encode(["name","email","phone","message","issue"]),
        ];
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Queries\Views\Querieslist', $mainDetails)
        ];
        return view('Core\Queries\Views\index', $data);
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Queries\Views\content', $addDatas)
        ];
        return view('Core\Queries\Views\index', $data);
    }
