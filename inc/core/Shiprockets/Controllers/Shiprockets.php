<?php
namespace Core\Shiprockets\Controllers;
use App\Controllers\PeginationController;

class Shiprockets extends \CodeIgniter\Controller
{
    public function __construct(){
        $this->config = include realpath( __DIR__."/../Config.php" );
        $this->model = new \Core\Shiprockets\Models\ShiprocketsModel();
        $this->db = \Config\Database::connect();
        $this->master = "shiprockets";
    }
    ...
    public function index()
    {
        ...
        $mainDetails = [   
            'dashboard' => $master,
            'title' => $master, 
            'heading' => "Shiprockets List", 
            'print' => '', 
            'searchname' => "Search $master", 
            'searching'=> '',
            'filterinput'=> '',
            'filterids'=>json_encode([]),
            'th'=>["Name","Email", "Password"],
            'mainid'=> "get$master",
            'addbtnroute'=> "$master/add",
            'routeURL'=> "$master/get",
            'editroute'=> "/$master/edit/",
            'deleteURL'=> "$master/deleteit",
            'td'=>json_encode(["name","email","password"]),
        ];
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Shiprockets\Views\Shiprocketslist', $mainDetails)
        ];
        return view('Core\Shiprockets\Views\index', $data);
    }
    ...
    public function Shiprocketssearch()
    {
        $searchMain = $this->request->getVar('allData');
    ...
    public function edit($id)
    {
        ...
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Shiprockets\Views\content', $addDatas)
        ];
        return view('Core\Shiprockets\Views\index', $data);
    }
