<?php
namespace Core\Reviews\Controllers;
use App\Controllers\PeginationController;

class Reviews extends \CodeIgniter\Controller
{
    public function __construct()
    {
        $this->config = include realpath(__DIR__ . "/../Config.php");
        $this->model = new \Core\Reviews\Models\ReviewsModel();
    ...
    public function index()
    {
        ...
        $mainDetails = [
            ...
            'th' => ["User Name", "Category", "Product", "Published"],
            'mainid' => "get$master",
            'addbtnroute' => "$master/add",
            'routeURL' => "$master/get",
            'editroute' => "/$master/edit/",
            'deleteURL' => "$master/deleteit",
            'td' => json_encode(["category_name", "product_name", "published"]),
        ];
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Reviews\Views\Reviewslist', $mainDetails)
        ];
        return view('Core\Reviews\Views\index', $data);
    }
    ...
    public function get()
    {
        ...
        foreach ($data as &$value) {
            $value['category_name'] = getInfoById('ramaeri_categories', $value['category'], 'name');
        }
        foreach ($data as &$value) {
            $value['product_name'] = getInfoById('ramaeri_products', $value['product'], 'name');
        }
    ...
    public function edit($id)
    {
        ...
        $addDatas = [
            ...
            'products' => isset($product) ? $product : [],
        ];
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\Reviews\Views\content', $addDatas)
        ];
        return view('Core\Reviews\Views\index', $data);
    }
