<?php
namespace Core\ProductBanners\Controllers;
use App\Controllers\PeginationController;

class ProductBanners extends \CodeIgniter\Controller
{
    public function __construct(){
        $this->config = include realpath( __DIR__."/../Config.php" );
        $this->model = new \Core\ProductBanners\Models\ProductBannersModel();
        $this->db = \Config\Database::connect();
        $this->master = "productBanners";
    }
    ...
    public function index()
    {
        ...
        $mainDetails = [   
            'dashboard' => $master,
            'title' => $master, 
            'heading' => "ProductBanners List", 
            'print' => '', 
            'searchname' => "Search $master", 
            'searching'=> '',
            'filterinput'=> '',
            'filterids'=>json_encode([]),
            'th'=>["Banners Image", "Last Modified","Published"],
            'mainid'=> "get$master",
            'addbtnroute'=> "$master/add",
            'routeURL'=> "$master/get",
            'editroute'=> "/$master/edit/",
            'deleteURL'=> "$master/deleteit",
            'td'=>json_encode(["updated_at","published"]),
        ];
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\ProductBanners\Views\ProductBannerslist', $mainDetails)
        ];
        return view('Core\ProductBanners\Views\index', $data);
    }
    ...
    public function get()
    {
        ...
        $data = $builder->where('product_image IS NOT NULL')->get()->getResultArray();
    ...
    public function ProductBannerssearch()
    {
        $searchMain = $this->request->getVar('allData');
    ...
    public function edit($id)
    {
        ...
        $data = [
            "title" => $this->config['name'],
            "desc" => $this->config['desc'],
            "content" => view('Core\ProductBanners\Views\content', $addDatas)
        ];
        return view('Core\ProductBanners\Views\index', $data);
    }
    ...
    {
       $data =  $this->request->getPost(index: "delete");
